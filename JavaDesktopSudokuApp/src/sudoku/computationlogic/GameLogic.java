...
public class GameLogic {
    ...
    public static GameState checkForCompletion(int[][] grid) {
        //If the Sudoku is invalid, or not all the tile have been filled in, return Active
     ...
     * @return
     */
    //Returns true if any of the tiles are not filled in
    public static boolean tilesAreNotFilled(int[][] grid) {
        for (int xIndex = 0; xIndex < GRID_BOUNDARY; xIndex++) {
            for (int yIndex = 0; yIndex < GRID_BOUNDARY; yIndex++) {
                // If the tile is 0 (empty), then return true
    ...
    public static boolean sudokuIsInvalid(int[][] grid) {
        //We use our methods from below to check if they are invalid
    ...
    public static boolean squaresAreInvalid(int[][] grid) {
        //top three squares
        //We call the rowOfSquaresIsInvalid() method from below
    ...
    // Returns true if any of the squares in the row is invalid
    private static boolean rowOfSquaresIsInvalid(Rows value, int[][] grid) {
        switch (value) {
            //For checking the top 3 squares
            case TOP:
                //x FIRST = 0
                //We call the squareIsInvalid() method fom below t check if the square is invalid
    ...
    //Checks if each individual square is invalid
    public static boolean squareIsInvalid(int yIndex, int xIndex, int[][] grid) {
        ...
        while (yIndex < yIndexEnd) {
            ...
            while (xIndex < xIndexEnd) {
                //We add the squares to the list
                square.add(
                        grid[xIndex][yIndex]
                );
                xIndex++;
        ...
        // We use our collectionHasRepeats() method we have created below
        if (collectionHasRepeats(square)) return true;
        return false;
    }

    // Returns true is the column is invalid
    ...
    // Returns true is the row is invalid
    ...
    //Its going to look at each square individually and check if a number has been repeated
