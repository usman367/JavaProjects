...
public class UserInterfaceImpl implements IUserInterfaceContract.View,
        EventHandler<KeyEvent> { // EventHandler comes from JavaFX
    ...
    // We store the TextFields inside the hashMap and we use coordinates to get them
    ...
    private void drawTextFields(Group root) {
        ...
        for (int xIndex = 0; xIndex < 9; xIndex++) {
            for (int yIndex = 0; yIndex < 9; yIndex++) {
                ...
                // We call the styleSudokuTile() we have created below for entering text
                ...
                // This is how we listen for inuts from the user
     ...
     * @param y
     */
    // For creating the style to enter the text into the individual tiles
    ...
    private void drawGridLines(Group root) {
        ...
        while (index < 8) {
            int thickness;
            //If the index is 2 or 5, we want the thickness of the line to be a little thicker
            if (index == 2 || index == 5) {
                thickness = 3;
            } else {
                thickness = 2;
            ...
            // We call the getLine func from below to create the line
            Rectangle verticalLine = getLine(
                    //We multiple it by the index so it separates the lines out
                    xAndY + 64 * index,
                    BOARD_PADDING,
                    BOARD_X_AND_Y,
                    thickness
     ...
     */
    // For creating the line, this func is used in the func above
    ...
    // For updating the whole board, when the user finishes the game
    @Override
    public void updateBoard(SudokuGame game) {
        for (int xIndex = 0; xIndex < 9; xIndex++) {
            for (int yIndex = 0; yIndex < 9; yIndex++) {
                ...
                //If the value is 0, the set it to an empty string
                if (value.equals("0")) value = "";
                tile.setText(
                        value
                );
    ...
    // Creating a custom dialog, for telling the user they've won, if they want to start a new game
    @Override
...
//    Comes form the event handler interface
    @Override
    public void handle(KeyEvent event) {
        // If a key is pressed and if the key is a number
        if (event.getEventType() == KeyEvent.KEY_PRESSED) {
            if (event.getText().equals("0")
                ...
                //Source is the object that was clicked on
                handleInput(value, event.getSource());
                //If a backspace is pressed, make the number 0, which will be converted into an empty string somewhere above
            } else if (event.getCode() == KeyCode.BACK_SPACE) {
                handleInput(0, event.getSource());
            } else {
                //If the user types anything else, set it to an empty string
    ...
    private void handleInput(int value, Object source) {
        // Once an input occurs , we pass it to our control logic class (listener)
        listener.onSudokuInput(
                // We tell it at this x and y value, this value was called
